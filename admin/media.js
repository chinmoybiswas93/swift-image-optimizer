...
function buildPanel( data, onAction ) {
	...
	if ( optimized ) {
		...
		if ( data.canRestore ) {
			const restore = el( 'button', 'button button-link sio-panel__restore', __( 'Restore original', 'swift-image-optimizer' ) );
			restore.type = 'button';
	...
	if ( data.canOptimize ) {
		...
		const button = el( 'button', 'button button-secondary sio-panel__optimize', label );
		button.type = 'button';
...
class OptimizeProgress {
	constructor( toolbarEl, total, onStop ) {
		...
		this.stop = el( 'button', 'button button-secondary sio-bulk__stop', __( 'Stop', 'swift-image-optimizer' ) );
...
function registerToolbar() {
	...
	const OptimizeModeButton = Button.extend( {
		...
		render() {
			...
			this.$el.addClass( 'swift-optimize-mode-button button-compact' );
			this.toggleVisible();
			return this;
		},
	} );
	...
	const OptimizeSelectedButton = Button.extend( {
		...
		render() {
			...
			this.$el.addClass( 'swift-optimize-selected-button' );
