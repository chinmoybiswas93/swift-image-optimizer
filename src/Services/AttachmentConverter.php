...
namespace SwiftImageOptimizer\Services;
...
use SwiftImageOptimizer\Services\Rewrite\UrlMap;
...
class AttachmentConverter {
	...
	private function do_convert( $attachment_id, $defer_rewrite ) {
		...
		$existing = Database::get( $attachment_id );

		if ( $existing && Database::STATUS_OPTIMIZED === $existing['status'] ) {
		...
		Database::remember_urls( $attachment_id, $url_map );
	...
	public function restore( $attachment_id ) {
		...
		Database::forget_urls( $attachment_id );
		...
		Database::upsert(
			$attachment_id,
			array(
				'status'        => Database::STATUS_RESTORED,
