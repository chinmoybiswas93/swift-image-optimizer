...
namespace SwiftImageOptimizer\Services\Diagnostics;
...
class EnvironmentReport {
	...
	private static function wordpress() {
		...
		$next_purge    = wp_next_scheduled( RetentionCron::HOOK );
	...
	private static function plugin() {
		...
		$rows = array(
			...
			self::row( __( 'Database schema version', 'swift-image-optimizer' ), (string) get_option( Database::SCHEMA_OPTION, 0 ), self::OK, '' ),
			...
			self::row( __( 'Optimized', 'swift-image-optimizer' ), (string) $counts[ Database::STATUS_OPTIMIZED ], self::OK, '' ),
			self::row( __( 'Skipped', 'swift-image-optimizer' ), (string) $counts[ Database::STATUS_SKIPPED ], self::OK, '' ),
			self::row(
				__( 'Failed', 'swift-image-optimizer' ),
				(string) $counts[ Database::STATUS_FAILED ],
				$counts[ Database::STATUS_FAILED ] > 0 ? self::WARN : self::OK,
				$counts[ Database::STATUS_FAILED ] > 0 ? __( 'Turn on logging and use Requeue below to try these again.', 'swift-image-optimizer' ) : ''
			),
			self::row( __( 'Restored', 'swift-image-optimizer' ), (string) $counts[ Database::STATUS_RESTORED ], self::OK, '' ),
			self::row(
	...
	private static function status_counts() {
		...
		$counts = array(
			Database::STATUS_OPTIMIZED => 0,
			Database::STATUS_SKIPPED   => 0,
			Database::STATUS_FAILED    => 0,
			Database::STATUS_RESTORED  => 0,
		);

		$table = Database::table();

		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Plugin-owned table; diagnostics screen, always read live.
