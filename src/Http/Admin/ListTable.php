...
class ListTable {
	...
	public function render_column( $column, $attachment_id ) {
		...
		$row = Database::get( $attachment_id );
		...
		if ( ! $row ) {
			...
			echo '<span class="swift-image-optimizer-pending">' . esc_html__( 'Not optimized', 'swift-image-optimizer' ) . '</span>';
			return;
		}

		switch ( $row['status'] ) {
			case Database::STATUS_OPTIMIZED:
				...
				printf(
					'<strong style="color:#008a20">%s</strong><br><span class="description">%s</span>',
					esc_html(
						sprintf(
			...
			case Database::STATUS_SKIPPED:
				printf(
					'<span class="description">%s</span>',
					esc_html( $this->reason_label( $row['reason'] ) )
				);
				break;

			case Database::STATUS_FAILED:
				printf(
					'<span style="color:#d63638">%s</span><br><span class="description">%s</span>',
					esc_html__( 'Failed', 'swift-image-optimizer' ),
					esc_html( $this->reason_label( $row['reason'] ) )
				);
				break;

			case Database::STATUS_RESTORED:
				printf( '<span class="description">%s</span>', esc_html__( 'Restored to original', 'swift-image-optimizer' ) );
				break;
	...
	public function add_row_actions( $actions, $post ) {
		...
		$row  = Database::get( $post->ID );
		$mime = get_post_mime_type( $post->ID );

		if ( $row && Database::STATUS_OPTIMIZED === $row['status'] ) {
			if ( BackupManager::manifest_is_intact( $row['backup_path'] ) ) {
				$actions['swift_restore'] = sprintf(
					'<a href="%s" class="swift-image-optimizer-action" data-action="restore" data-id="%d">%s</a>',
		...
		$actions['swift_optimize'] = sprintf(
			'<a href="%s" class="swift-image-optimizer-action" data-action="optimize" data-id="%d">%s</a>',
	...
	public function handle_bulk_action( $redirect, $action, $ids ) {
		$converter = swift_image_optimizer()->converter();
	...
	private function maybe_handle_row_action() {
		...
		printf(
			'<div class="notice notice-success is-dismissible"><p>%s</p></div>',
			'optimize' === $action
				? esc_html__( 'Image optimized.', 'swift-image-optimizer' )
				: esc_html__( 'Original restored.', 'swift-image-optimizer' )
		);
	}
	...
	public function expose_to_js( $response, $attachment ) {
		...
		$row         = Database::get( $attachment->ID );
		...
		$response['swiftImageOptimizer'] = array(
			...
			'canOptimize'   => $editable && $convertible && Database::STATUS_OPTIMIZED !== $status,
